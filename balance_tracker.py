...
class BalanceTracker:
    ...
    def print_account_information(self):
        for account in self._response["accounts"]:
            print(
                account["account_id"], account["name"], account["balances"]["available"]
            )
...
class SchwabTracker:
    def __init__(self, plaid_client, schwab_access_token):
        self.plaid_client = plaid_client
        self.schwab_access_token = schwab_access_token

    def current_balance(self) -> float:
        pl = self.plaid_client
        response = pl.Accounts.balance.get(self.schwab_access_token)
